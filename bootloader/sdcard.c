...
static volatile uint8_t sd_sdio_transfer_complete = 0;
static volatile uint8_t sd_transfer_error = 0;
static volatile uint8_t sd_cmd_irq_flag = 0;
...
#define DISABLE_STATIC_IRQ()				(SDIO->MASK &= 0x5FF)
#define CLEAR_STATIC_IRQ()				(SDIO->ICR = 0x5FF)
...
static void printError(const char* function, SD_Status code, const char* extra)
{
	...
	if (extra)
		debugPrint(DebugError, "SdDebug: error \"%s\" in function %s [%s]", getErrorString(code), function, extra);
	else
		debugPrint(DebugError, "SdDebug: error \"%s\" in function %s", getErrorString(code), function);
}
...
static void printErrorRWOp(bool write, SD_Status code, uint32_t sector, uint32_t count, uint32_t retries, const char* msg)
{
	...
	if (msg)
		debugPrint(DebugError, "SdDebug: error \"%s\" in %s operation, sector %lu, count %lu [%s]", getErrorString(code), write ? "write" : "read", sector, count, msg);
	else
		debugPrint(DebugError, "SdDebug: error \"%s\" in %s operation, sector %lu, count %lu", getErrorString(code), write ? "write" : "read", sector, count);

	debugPrint(DebugInfo,"SdDebug: retries left = %lu", retries);
}
...
static void sdInitializeSDIO(uint8_t mode, uint8_t wide)
{
...
{
#if defined(BOOTLOADER_PROPBOARD)
	return (GPIOA->IDR & GPIO_Pin_8) == 0;
#elif defined(BOOTLOADER_WAVETOOEASY)
	return (GPIOA->IDR & GPIO_Pin_15) == 0;
#endif
}
...
SD_Status sdInitialize(void)
{
	...
	sdInitializeDMA();
	sdInitializeSDIO(SDIO_MODE_INITIALIZATION, 0);	/* Low speed */
...
SD_Status sdGetStatus(uint8_t lock)
{
	SD_Status status;
	SDIO_CmdInitTypeDef sdio_cmd;
...
static SD_Status sdSendBlockRxCmd(uint32_t sector, uint8_t multi)
{
	SDIO_CmdInitTypeDef sdio_cmd;
	SD_Status status;
...
static SD_Status sdSendBlockTxCmd(uint32_t sector, uint8_t multi)
{
	SDIO_CmdInitTypeDef sdio_cmd;
	SD_Status status;
...
static uint8_t sdTransferDone()
{
	uint32_t response;
	SD_Status status;

	status = sdGetStatus(0);
...
static uint8_t sdAbortTransmission(uint8_t multi)
{
	uint32_t ticks;
...
static SD_Status sdTransferBlocksWithDMA(uint32_t sector, const uint8_t* buffer, uint32_t count, uint8_t write)
{
	...
	while (retries)
	{
		...
		sd_cmd_irq_flag = sd_transfer_error = sd_sdio_transfer_complete = 0;
...
SD_Status sdReadBlocks(uint32_t sector, uint8_t* buffer, uint32_t count)
{
	...
	ret = sdTransferBlocksWithDMA(sector, buffer, count, 0);
...
SD_Status sdWriteBlocks(uint32_t sector, const uint8_t* buffer, uint32_t count)
{
	...
	ret = sdTransferBlocksWithDMA(sector, buffer, count, 1);
...
void sdPrintStats()
{
	debugMsg(DebugInfo, "\r\nSD statistics");
	debugMsg(DebugInfo, "----------------");
	debugMsg(DebugInfo, "Errors: %lu", sd_errors);
	debugMsg(DebugInfo, "Timeouts: %lu", sd_timeouts);
	debugMsg(DebugInfo, "Max. retries: %lu", sd_max_retries);
	debugMsg(DebugInfo, "Sectors read: %lu", sd_sectors_read);
	debugMsg(DebugInfo, "Average read time: %lu uS", sd_avg_read_time);
	debugMsg(DebugInfo, "SD errors: %lu", sd_errors);
	debugMsg(DebugInfo, "Max. read time: %lu uS", sd_max_read_time);
	debugMsg(DebugInfo, "Sectors written: %lu", sd_sectors_written);
	debugMsg(DebugInfo, "Average write time: %lu uS", sd_avg_write_time);
	debugMsg(DebugInfo, "Max. write time: %lu", sd_max_write_time);
}
#endif // SD_STATS
