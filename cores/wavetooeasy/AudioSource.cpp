...
{
	setVolume(value, VOLUME_CHANGE_SAMPLES);
}

void AudioSource::setVolume(float value, uint32_t samples)
{
	if (!samples)
		return;
	...
	if (value == current_volume)
	{
		...
		{
			target_volume = value;
			target_volume_samples = samples;
			target_volume_step = (current_volume - value) / target_volume_samples;
		} else {
