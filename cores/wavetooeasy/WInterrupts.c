...
typedef void (*interruptCBWP)(void*);

static interruptCB irq_funcs[16] = {0};
static interruptCBWP irq_funcswp[16] = {0};
static void* irq_params[16] = {0};
...
static void attachInterruptInternal(uint32_t pin,
									void (*callback)(void),
									void (*callbackwp)(void*),
									uint32_t mode,
									void* param)
{
	...
	if (param)
	{
		irq_funcswp[g_APinDescription[pin].irq_num] = callbackwp;
		irq_params[g_APinDescription[pin].irq_num] = param;
	} else {
...
{
	attachInterruptInternal(pin, callback, NULL, mode, NULL);
}

void attachInterruptWithParam(uint32_t pin, void (*callback)(void*), uint32_t mode, void* param)
{
	attachInterruptInternal(pin, NULL, callback, mode, param);
}
...
void detachInterrupt(uint32_t pin)
{
	...
	__disable_irq();
	if (irq_params[g_APinDescription[pin].irq_num])
	{
		irq_params[g_APinDescription[pin].irq_num] = NULL;
		irq_funcswp[g_APinDescription[pin].irq_num] = NULL;
	} else {
...
static void EXTI_IRQHandler(uint32_t line_num)
{
	if (EXTI->PR & (1 << line_num))
	{
		...
		if (irq_params[line_num])
		{
			if (irq_funcswp[line_num])
				(irq_funcswp[line_num])(irq_params[line_num]);
		} else {
