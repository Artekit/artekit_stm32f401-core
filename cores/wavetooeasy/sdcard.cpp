...
static SD_Status sdStartAndSelect(void)
{
	...
	{
		card_info.capacity = 0;
		card_info.capacity = (uint64_t) ((uint32_t) (card_info.csd.fields_v2.c_sizel | (card_info.csd.fields_v2.c_sizeh << 16)) + 1) * (uint32_t) 512 * (uint32_t) 1024;
...
*/
	...
	if (write)
	{
		...
		if (sd_transfer_error_code == SD_NO_ERROR)
		{
			last_read_time = micros() - transfer_time;
			if (last_read_time > sd_max_read_time)
				sd_max_read_time = last_read_time;

			if (count)
				sd_avg_read_time = ((sd_sectors_read * sd_avg_read_time) + last_read_time) / (sd_sectors_read + count);
